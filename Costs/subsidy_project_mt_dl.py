...
def update_project_subsidy(auth_token: str, payload: Dict[str, Any]) -> Tuple[int, str]:
    """Обновляет субсидию проекта через POST-запрос."""
...
def prepare_payload(project_data: Dict[str, Any], executor_rate_with_subsidy: str) -> Dict[str, Any]:
    """Формирует payload для обновления субсидии проекта."""
    rates_data = project_data["rates"]["perHourRates"][0]["id"]
    executor_rate = project_data["rates"]["perHourRates"][0]["executorFinalRate"]
...
def process_line(line: str, auth_token: str, total: int):
    ...
    try:
        ...
        with lock:
            print(f'✅ {project_id} | Статус: {status} | Ответ: {response[:50]}...')
